...
function Start() {
	...
	ready = true;
	velocityRollX = Random.Range(-200,200);
	velocityRollY = Random.Range(-200,200);
...
function OnTriggerEnter(collision : Collider) {
	if(collision.gameObject.tag == "Projectile" && !isShuttingDown && !isHit && transform.position.z > -10) {
		...
		LootManager.LootMultiplier(xPosition - 2, yPosition - 2);
   ...
   if(collision.gameObject.tag == "PlayerShip" && !isShuttingDown && !Menu.newGame) {
		Menu.PlayerDied();
		Debug.Log("Tutorial Guy Killed you.");
	} 
}
