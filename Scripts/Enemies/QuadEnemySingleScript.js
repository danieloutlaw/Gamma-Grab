...
//	transform.rigidbody.velocity = transform.up * velocityRoll;
	for( ; ; ) {
		...
		if(transform.parent == null && !isAloneStart) {
			isAloneStart = true;
...
function Update () {
	if(transform.parent == null && !colorChanged) {
		colorChanged = true;
...
function FixedUpdate(){
	...
	transform.rigidbody.velocity = transform.up * 250;
...
function OnTriggerEnter(collision : Collider) {
	if(collision.gameObject.tag == "Projectile" && !isShuttingDown && !isHit && transform.position.z > -10 && transform.parent == null && isAlone) {
		isHit = true;
	...
	if(collision.gameObject.tag == "PlayerShip" && !isShuttingDown && !Menu.newGame) {
		Menu.PlayerDied();
		Debug.Log("Single Quad Enemy Killed you.");
	}
